...
public class T1_skip {
	...
	/* constructor */
	public T1_skip(String inputFile, String verifyFile, double mst, int tableSize) {
	...
	public void runSkipLCSS(String inputFile, String verifyFile, double mst, int tableSize) throws IOException {
		...
		double t1_threshold = (Math.log(tableSize) / Math.log(2)) + 1;
		...
		while ((line = br.readLine()) != null) {			
			...
			updateSummary(tran, hasAppearedInTable, t1_threshold );
//			entryTable.printItemsets(1);
		}

		// print info
		System.out.println("t1 skip number : " + skipNum);
		System.out.println("t1_threshold : " + t1_threshold);
		this.printSystemInfo();
		this.printSummaryInfo();
		// mining
		miningResult = miningSummary(entryTable, mst * num, skipNum);
		// verify
	...
	public void updateSummary(int[] transaction, ArrayList<int[]> hasAppearedInTable, double t1_threshold) {
		hasAppearedInTable = new ArrayList<int[]>();
		
		// the key point of t1-skip
		if((double)transaction.length > t1_threshold ){ // it is case C
			skipNum++; // record the skip number
			System.out.println("t1 skip");
	...
	public ArrayList<int[]> miningSummary(EntryTable table, double threshold, int skipNum) {
		ArrayList<int[]> list = new ArrayList<int[]>();
		// check each entry
		for (Itemset itemset : table.getItemsets()) {
			if (itemset.getAbsoluteSupport() + skipNum >= threshold) {
